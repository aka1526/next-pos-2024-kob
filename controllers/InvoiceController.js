...
exports.report1 = async (req, res) => {
  ...
  //ตำแหน่ง 240, 30
    ...
    // doc.rect(padding, padding, paperWidth - 2 * padding, paperHieght - 2 * padding).stroke();
     ...
     // วาดเส้นขอบมน
   doc.lineWidth(0.6);
   doc.roundedRect(herderMargin, herderMargin, paperWidth - 2 * herderMargin, paperHieght- 2 * herderMargin, cornerRadius).stroke();
  
     
    doc.font(AngsanaBold);
    doc.fontSize(20).text("ใบเบิก-จ่ายวัสดุสิ้นเปลือง", 200, doc.y + 8);
    doc.font(Angsana);
    doc.fontSize(font12).text('หน้า   '+(parseInt(index+1).toString()), 460, 16, {align: 'right',width: 100});
    doc.fontSize(font12).text(`เลขที่เอกสาร :`,480, 30,{ align: 'left', width: 150 });
    doc.font(AngsanaBold);
    doc.fontSize(font12).text(` ${docno}`,530, 30,{ align: 'left', width: 150 });
    doc.font(Angsana);
    doc.fontSize(font12).text(`วันที่ : ${dayjs(new Date()).format("DD/MM/YYYY HH:mm:ss")}`,480, doc.y+2,{ align: 'left', width: 150 });
    doc.moveDown();
}
...
function generateTableRowHeader(doc ) {
  ...
  doc.rect(506, 66, 73, parseInt(font16)+2).stroke();
   ...
   function generateTableRow(doc, y, c1, c2, c3, c4,c5,c6) {
       ...
       doc.text(c2 , 46+4, y, { align: 'left' , width: 120-10, pad : 10});
       doc.text(c3 , 166+4, y, { align: 'right' , width: 200-10,padding : 20});
...
billSaleDetails.map((row, index) => {
    ...
    if (position > 300) {  //  ตำแหน่งขึ้นหน้าใหม่
        doc.addPage();
        page=page+1;
